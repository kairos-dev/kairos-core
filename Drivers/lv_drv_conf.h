...
typedef i2c_dev_t lv_i2c_handle_t;
...
static inline int lv_i2c_write(lv_i2c_handle_t i2c_dev, const uint8_t* reg, const void* data_out, uint16_t datalen)
{
    ...
    i2c_master_write_byte(cmd, ( i2c_dev.addr << 1 ) | I2C_MASTER_WRITE, 1);
    i2c_master_write_byte(cmd, (*reg)&0xFF, 1);
    // for(uint16_t length = 0; length < datalen; length++)
    // {
    i2c_master_write(cmd, data, datalen, 1);
    // }
    i2c_master_stop(cmd);
    esp_err_t ret = i2c_master_cmd_begin(i2c_dev.bus, cmd, 1000 / portTICK_RATE_MS);
...
static inline int lv_i2c_read(lv_i2c_handle_t i2c_dev, const uint8_t* reg, void* data_in, uint16_t datalen)
{
	...
	i2c_master_write_byte(cmd, (i2c_dev.addr << 1) | I2C_MASTER_READ, 1);
	i2c_master_read(cmd, data_in, datalen,1);
	i2c_master_stop(cmd);
